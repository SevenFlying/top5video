    ...
    <table cellspacing="0">
      <thead>
        <tr>
          ...
          <?php include_partial('video/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <tbody>
        <?php foreach ($pager->getResults() as $i => $Video): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          <tr class="sf_admin_row <?php echo $odd ?>">
            <?php include_partial('video/list_td_batch_actions', array('Video' => $Video, 'helper' => $helper)) ?>
            <?php include_partial('video/list_td_tabular', array('Video' => $Video)) ?>
