...
/**
 * Base class that represents a row from the 'score' table.
 *
 * 
 *
...
abstract class BaseScore extends BaseObject  implements Persistent {
	...
	public function setVideoId($v)
	{
		...
		if ($this->video_id !== $v) {
			$this->video_id = $v;
			$this->modifiedColumns[] = ScorePeer::VIDEO_ID;
		}
	...
	/**
	 * Set the value of [stars] column.
	 * 
	 * @param      int $v new value
	 * @return     Score The current object (for fluent API support)
	 */
	public function setStars($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}
		...
		if ($this->stars !== $v) {
			$this->stars = $v;
			$this->modifiedColumns[] = ScorePeer::STARS;
		}
	...
	public function setSlug($v)
	{
		...
		if ($this->slug !== $v) {
			$this->slug = $v;
			$this->modifiedColumns[] = ScorePeer::SLUG;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->stars = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			...
			return $startcol + 4; // 4 = ScorePeer::NUM_COLUMNS - ScorePeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseScore:delete:pre') as $callable)
			{
			...
			if ($ret) {
				ScorePeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseScore:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseScore:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseScore:save:post') as $callable)
				{
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = ScorePeer::doInsert($this, $con);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setStars($arr[$keys[2]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ScorePeer::VIDEO_ID)) $criteria->add(ScorePeer::VIDEO_ID, $this->video_id);
		if ($this->isColumnModified(ScorePeer::STARS)) $criteria->add(ScorePeer::STARS, $this->stars);
		if ($this->isColumnModified(ScorePeer::SLUG)) $criteria->add(ScorePeer::SLUG, $this->slug);
	...
	public function getVideo(PropelPDO $con = null)
	{
		if ($this->aVideo === null && ($this->video_id !== null)) {
			$this->aVideo = VideoPeer::retrieveByPk($this->video_id);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aVideo->addScores($this);
			 */
		}
		return $this->aVideo;
	}
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseScore:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseScore::%s', $method));
	  }
